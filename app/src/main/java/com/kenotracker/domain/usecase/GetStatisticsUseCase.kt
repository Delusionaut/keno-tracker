...
class GetStatisticsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): Statistics {
        ...
        val totalNumbers = draws.sumOf { it.numbers.size }
        val averageHits = if (totalDraws > 0) totalNumbers.toFloat() / totalDraws else 0f

        val hotNumbers = calculateHotNumbers(draws)
        val coldNumbers = calculateColdNumbers(draws)
        val trendNumbers = calculateTrendNumbers(draws)
    ...
    private fun calculateHotNumbers(draws: List<Draw>, count: Int = 9): List<NumberRecommendation> {
        ...
        return frequencyMap.entries
            ...
            .map { (number, hits) ->
                NumberRecommendation(
                    number = number,
                    confidence = hits.toFloat() / maxHits,
                    hitCount = hits,
                    hitRate = hits.toFloat() / draws.size * 100,
                    reason = "Hit $hits times"
                )
            }
    }

    private fun calculateColdNumbers(draws: List<Draw>, count: Int = 9): List<NumberRecommendation> {
        if (draws.isEmpty()) return emptyList()
        ...
        val drawnNumbers = frequencyMap.keys
        val neverDrawn = allNumbers - drawnNumbers
        ...
        return sorted.map { (number, hits) ->
            NumberRecommendation(
                number = number,
                confidence = if (hits == 0) 0.1f else 0.3f,
                hitCount = hits,
    ...
    private fun calculateTrendNumbers(draws: List<Draw>, count: Int = 9): List<NumberRecommendation> {
        if (draws.isEmpty()) return emptyList()
    ...
    private fun calculateRecencyScore(number: Int, sortedDraws: List<Draw>): Float {
        var weight = 1.0f
        var score = 0f
    ...
    private fun calculateGapScore(number: Int, sortedDraws: List<Draw>): Float {
    ...
    private fun calculateFrequencyScore(number: Int, draws: List<Draw>): Float {
    ...
    private fun calculateClusterScore(number: Int, sortedDraws: List<Draw>): Float {
